...
- **Temporary vs. Global:** For temporary use, prefer `nix-shell -p package` to avoid unnecessary global installations. Reserve `nix-shell -iA package` for less frequent scenarios.
...
### Direnv
**Automatic Setup:** Add either `use flakes` or `use nix` to your `.envrc` file for seamless environment loading when using direnv.
